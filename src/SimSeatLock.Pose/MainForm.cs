...
public sealed class MainForm : Form
{
        ...
        string geomPath)
    {
        ...
        {
            AutoSize = true,
            Text = "Preview inv(T_rig) (off = identity, same as headset)",
        ...
        var virtGrid = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 6, Padding = new Padding(6) };
        virtGrid.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize));
        virtGrid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        _roll = AddNud(virtGrid, 0, "roll deg", -180, 180, 0.05m, 2);
        _pitch = AddNud(virtGrid, 1, "pitch deg", -180, 180, 0.05m, 2);
        _yaw = AddNud(virtGrid, 2, "yaw deg", -180, 180, 0.05m, 2);
        _surge = AddNud(virtGrid, 3, "surge m", -2, 2, 0.001m, 4);
        _sway = AddNud(virtGrid, 4, "sway m", -2, 2, 0.001m, 4);
        _heave = AddNud(virtGrid, 5, "heave m", -2, 2, 0.001m, 4);
    ...
    static NumericUpDown AddNud(TableLayoutPanel grid, int row, string label, decimal min, decimal max, decimal inc, int places)
    {
    ...
    void RefreshLive()
    {
        ...
        _deltaBox.Text = FormatDelta(delta);
    ...
    string FormatAdjusted(in RigidPose p)
    {
        ...
        return
            "Adjusted T_view = T_cor * inv(T_rig) * inv(T_cor) * T_hmd  [identity when preview off]\r\n" +
            $"Adjusted ({mode})  space={p.Space}\r\n" +
            FormatRigid(p, e);
    }

    static string FormatDelta(in RigidPose p)
    {
        var e = p.ToEulerSample();
        return
            "Delta expected headset vs adjusted (= T_rig when preview on)\r\n" +
            $"Delta  {LiveHold(p.Live)}  should be ~identity  space={p.Space}\r\n" +
            FormatRigid(p, e);
    }
    ...
    string FormatRig(in PoseSample raw, in PoseSample home, in PoseSample rig)
    {
        return
            ...
            $"sg={home.SurgeM:0.000} sw={home.SwayM:0.000} hv={home.HeaveM:0.000}\r\n" +
            ...
            $"valid={rig.Valid}\r\n" +
            $"wit  {_serial.Status}  live={_serial.IsLive}";
    }

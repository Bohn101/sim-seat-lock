...
public sealed partial class MainForm : Form
{
        ...
        string geomPath)
    {
        ...
        _statusPad = new Panel { Dock = DockStyle.Bottom, Height = 28, BackColor = SystemColors.Control };
        _statusBar = new Panel { Dock = DockStyle.Bottom, Height = 32, BackColor = SystemColors.Control, Padding = new Padding(10, 4, 16, 4) };
        _status = new Label { Dock = DockStyle.Fill, AutoSize = false, AutoEllipsis = true, TextAlign = ContentAlignment.MiddleLeft, UseMnemonic = false };
        _statusBar.Controls.Add(_status);
        Controls.Add(tabs);
        Controls.Add(_statusBar);
        Controls.Add(_statusPad);

        var top = new FlowLayoutPanel { Dock = DockStyle.Top, AutoSize = true, WrapContents = true, Padding = new Padding(6, 4, 6, 2) };
        _preview = new CheckBox { AutoSize = true, Text = "Arm Layer / Preview inv(T_rig) — Off = identity", Checked = _previewArmed };
        _preview.CheckedChanged += (_, _) => { _previewArmed = _preview.Checked; _cfg.PreviewArmed = _previewArmed; };
        _home = new Button { Text = "Home IMU (Z)", AutoSize = true };
        _home.Click += (_, _) => TryHome();
        new ToolTip().SetToolTip(_home, "Zero the IMU offset at SimTools neutral. Not actuator home.");
        ...
        top.Controls.AddRange(new Control[] { _preview, _home, _zeroVirtual, _lampImu, _lampSteam, _lampLayer, _lampEngine });
        ...
        var virtGrid = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 4, RowCount = 4, Padding = new Padding(8) };
        ...
        _ui.Start();
        _pubThread = new Thread(PublishLoop) { IsBackground = true, Name = "SimSeatLock.Rig.pub", Priority = ThreadPriority.AboveNormal };
        _pubThread.Start();
    }

    static Label MakeLamp(string name) => new() { AutoSize = true, Text = "\u25cf " + name, Padding = new Padding(10, 6, 4, 0), ForeColor = Color.Gray };
    static void SetLamp(Label lamp, bool on, string extra) { lamp.ForeColor = on ? Color.ForestGreen : Color.Firebrick; lamp.Text = "\u25cf " + extra; }

    void BuildHelpTab(TabPage page)
    {
        page.Padding = new Padding(8);
        page.Controls.Add(new TextBox { Multiline = true, ReadOnly = true, Dock = DockStyle.Fill, ScrollBars = ScrollBars.Both, WordWrap = false, Font = new Font("Consolas", 9.5f), Text = HelpText.Body.Replace("\n", "\r\n"), BorderStyle = BorderStyle.None, BackColor = SystemColors.Window });
    }

    void BuildConfigTab(TabPage page)
    {
        var root = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 16, Padding = new Padding(10), AutoScroll = true, AutoSize = true };
        ...
        int r = 0;
        void LabelRow(string text) => root.Controls.Add(new Label { Text = text, AutoSize = true, Anchor = AnchorStyles.Left, Margin = new Padding(0, 8, 8, 4) }, 0, r);

        LabelRow("Source Mode");
        _srcBox = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList, Width = 200, Anchor = AnchorStyles.Left };
        ...
        {
            if (!_portBox.Items.Contains(_cfg.Witmotion.Port)) _portBox.Items.Insert(0, _cfg.Witmotion.Port);
        ...
        portRow.Controls.AddRange(new Control[] { _portBox, refreshPorts, applyPort });
        ...
        _gamesBox = new TextBox { Text = string.Join(Environment.NewLine, _cfg.GameProcesses), Multiline = true, Height = 96, Width = 360, ScrollBars = ScrollBars.Vertical, Font = new Font("Consolas", 9f) };
        ...
        savePose.Click += (_, _) => { ApplySourceMode(restartSerial: true); _cfg.Save(_cfgPath); _game.Watch.SetNames(_cfg.GameProcesses); _homeNote = $"saved {_cfgPath}"; };
        var saveGeom = new Button { Text = "Save geometry.json", AutoSize = true };
        saveGeom.Click += (_, _) => { PullGeomFromUi(); PoseConfig.SaveGeometry(_geomPath, _geom); _homeNote = $"saved {_geomPath}"; };
        ...
        void ApplyLive(object? _, EventArgs e) { PullPoseFromUi(); _pose.Gains = _cfg.ToGains(); }
    ...
    string SelectedSource() => string.Equals(_srcBox.Text, "Virtual Numeric", StringComparison.OrdinalIgnoreCase) ? "virtual" : "witmotion";
    ...
    void RefreshPorts()
    {
        ...
        foreach (var p in WitmotionSerialPoseSource.ListPorts()) _portBox.Items.Add(p);
        if (!string.IsNullOrWhiteSpace(keep))
        {
            if (!_portBox.Items.Contains(keep)) _portBox.Items.Insert(0, keep);
            _portBox.Text = keep;
        }
    }
    void AddRunningProcess()
    {
        var names = Process.GetProcesses().Select(p => { try { return p.ProcessName; } catch { return ""; } }).Where(n => n.Length > 0).Distinct(StringComparer.OrdinalIgnoreCase).OrderBy(n => n, StringComparer.OrdinalIgnoreCase).ToArray();
        using var pick = new Form { Text = "Add Running Process", StartPosition = FormStartPosition.CenterParent, ClientSize = new Size(420, 480), MinimizeBox = false, MaximizeBox = false };
        ...
        ok.Click += (_, _) =>
        {
            ...
            {
                var cur = _gamesBox.Text.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim()).ToHashSet(StringComparer.OrdinalIgnoreCase);
                if (cur.Add(n)) _gamesBox.Text = string.Join(Environment.NewLine, cur.OrderBy(s => s, StringComparer.OrdinalIgnoreCase));
            }
            pick.Close();
        };
    ...
    static NumericUpDown TinyGain(decimal v) => new() { DecimalPlaces = 2, Increment = 0.05m, Minimum = -4, Maximum = 4, Value = Math.Clamp(v, -4, 4), Width = 70 };
    void PullPoseFromUi()
    {
    ...
    static TextBox MakeBox() => new() { Multiline = true, ReadOnly = true, WordWrap = false, ScrollBars = ScrollBars.Both, Font = new Font("Consolas", 9f), Dock = DockStyle.Fill, BorderStyle = BorderStyle.FixedSingle, TabStop = false };
    static NumericUpDown AddNud(TableLayoutPanel grid, int col, int row, string label, decimal min, decimal max, decimal inc, int places)
    {
        ...
        var n = new NumericUpDown { Minimum = min, Maximum = max, Increment = inc, DecimalPlaces = places, Dock = DockStyle.Fill, ThousandsSeparator = false, Margin = new Padding(0, 4, 8, 4) };
